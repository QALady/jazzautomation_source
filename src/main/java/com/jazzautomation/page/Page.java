 ...
 * navigation from one this page to a new page, gotoPage needs to be called. Typical sequences are:
 */
...
public class Page
{
  ...
  @XmlTransient
  private float                            pageLoadTimeout   = 10.0f;  // default value 10 seconds
  @XmlTransient
  ...
  private boolean handleVisibleAction(DomElementExpectation expect, DomElement domElement, boolean expectationMet)
  {
    ...
    boolean    isNotVisible = !((webElement != null) && webElement.isDisplayed() && webElement.isEnabled());

    if (isNotVisible && !expect.isNegative())
    {
      expectationMet = false;
  ...
  private boolean handleEqualsAction(DomElementExpectation expect, DomElement domElement, boolean expectationMet, String htmlValue)
  {
    ...
    if (Utils.isANumber(expect.getValue()))
    {
      ...
      if (Math.abs(expectValue - actualValue) > 0.00001d)  // if they differ by more than a small amount...
      {
        expectationMet = false;
  ...
  private HtmlActionStatus tryLoadDomElement(final DomElement domElement, int numTry)
  {
    ...
    {
      LOG.error("Can not load web component [" + domElement.getIdentifier() + "] after " + (NUMBER_OF_RETRY + 1) + "] tries."
  ...
  private Action instantiateCustomAction(Class clazz) throws InstantiationException, IllegalAccessException, NoSuchMethodException,
  ...
  public DomElement getDomElement(String componentName)
  {
    ...
    DomElement domElement = domElements.get(componentName + '-' + browser.getLowercaseName());
    ...
    if (domElement == null)
    {
      ...
      domElement = WebUIManager.getInstance().getDomElementFromPool(componentName + '-' + browser.getLowercaseName());
